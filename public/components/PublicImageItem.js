      ...
      {data.map((item, index) => (
        <View style={styles.checkListItem} key={item.id}>
          ...
          <Image
            source={{
              uri:
                getProtocol() + (item.img_url ? item.img_url : item.avatar_url),
            }}
            style={[
              styles.stuAvatar,
              {marginLeft: avatarMarginLeft ? avatarMarginLeft : 20},
            ]}
          />
          <Text>{item.name}</Text>
          <TouchableOpacity
            ...
            <Text>
              {rightName
                ...
                : checkItem[item.checkTipsId].name}
            </Text>
          </TouchableOpacity>
        </View>
      ))}
