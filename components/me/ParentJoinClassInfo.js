...
class ParentJoinClassInfo extends Component {
  ...
  render() {
    ...
    return (
      <View>
        <PublicHeader title="验证消息" navigation={navigation} isLeft={true} />
        <View>
          {selectIdentity === 0 ? (
            parentJoinClassInfo.length > 0 ? (
              parentJoinClassInfo.map(item => {
                return (
                  <View style={styles.parentJoinClassInfoItem} key={item.id}>
                    <Image
                    ...
                    <View>
                      <Text>
                        {item.username}({item['student_name']})
                      </Text>
                      <Text style={styles.applyText}>
                        申请加入{item['class_grade']}
                        {item['class_name']}
                      </Text>
                    </View>
                    {item['parent_in_class'] === '0' ? (
                      <TouchableOpacity
                        ...
                        <Text style={styles.willAllowText}>通过</Text>
                      </TouchableOpacity>
                    ) : (
