          ...
          <View style={{display: 'flex', alignItems: 'center'}}>
            <View style={styles.detailContainer}>
              <View style={styles.detailItem}>
                ...
                />
                <Text style={styles.detailText}>全班</Text>
              </View>
              {studentList.map(item => (
                <TouchableOpacity
                  ...
                  <Image
                    source={
                      item.avatar_url.indexOf('img') === -1
                        ? {uri: getProtocol() + item.avatar_url}
                        : getLocalImgIndex('student', item.avatar_url)
                    }
                    style={styles.stuAvatar}
                  />
                  <Badge
              ...
              <View>
                ...
                isMaster === 1 ? (
                  <TouchableOpacity
                    onPress={this.handleAddStu}
                    style={styles.detailItem}>
                    <View style={[styles.stuAvatar, styles.addStu]}>
                      <Icon name="add" color="#00aced" size={34} />
                    </View>
                    <Text style={styles.detailText}>添加学生</Text>
                  </TouchableOpacity>
                ) : null}
              </View>
            </View>
    ...
    return (
      <View style={{height: '100%'}}>
        <PublicScrollView
          ...
          updateFun={updateFun}
        />
        {studentList.length === 0 ? null : isMaster === 0 ? (
          <View style={styles.bottomBtns}>
            <TouchableOpacity
              style={[styles.bottomBtn]}
              onPress={this.showRandom}>
              <Icon name="repeat" color="#70768c" />
              <Text>随机</Text>
            </TouchableOpacity>
          </View>
        ) : null}
