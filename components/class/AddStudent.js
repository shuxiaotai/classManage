...
class AddStudent extends Component {
  ...
  addStudentName = () => {
    ...
    if (studentName === '') {
      ...
      let student = {
        name: studentName,
        avatar_url: `/img/student${Math.floor(
          Math.random() * (1 - 14) + 14,
        )}.jpg`,
      };
    ...
    return (
      <View style={{marginBottom: 100}}>
        ...
        <View style={styles.inputContainer}>
          ...
          />
          <Text style={styles.addStuText} onPress={this.addStudentName}>
            添加
          </Text>
        </View>
        <ScrollView contentContainerStyle={styles.stuList}>
          {addStudentList.map((item, index) => {
            return (
              <View style={styles.stuItem} key={index}>
                ...
                />
                <Text style={styles.stuText}>{item.name}</Text>
              </View>
            );
          })}
        </ScrollView>
