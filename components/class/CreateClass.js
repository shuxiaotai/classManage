...
class CreateClass extends Component {
  ...
  getId = (list, value) => {
    const {
      setDefaultPraiseIds,
      setDefaultCriticizeIds,
      setDefaultScheduleIds,
      setDefaultCourseIds,
    } = this.props;
    let arr = [];
    list.forEach(item => {
  ...
  render() {
    ...
    return (
      <View>
        <PublicHeader title="创建班级" isLeft={true} navigation={navigation} />
        ...
        <PublicHorizontalItem
          leftText="导入点评"
          rightText={
            defaultPraiseIds.length !== 0 || defaultCriticizeIds.length !== 0
              ? '已选择'
              : '请选择'
          }
        ...
        <PublicHorizontalItem
          leftText="导入项目"
          rightText={
            defaultScheduleIds.length !== 0 || defaultCourseIds.length !== 0
              ? '已选择'
              : '请选择'
          }
          toTargetFun={this.toImportProject}
        />
        <PublicBtn tips="创建" onPress={this.toAddClass} />
      </View>
    );
  }
}
