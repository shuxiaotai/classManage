...
class StudentHomePage extends Component {
  ...
  toGetRightComponent = () => {
    return <Text style={styles.stuInfo}>学生资料</Text>;
  };
  toStudentDetailInfo = () => {
    const {navigate} = this.props.navigation;
      ...
      <View style={styles.stuRemarkItem}>
        ...
        <View>
          <View style={[styles.rightTips, styles.topScore]}>
            ...
            <Text>
              因为 {item.group_name}{' '}
              {item.project_name === null ? '自定义点评' : item.project_name}{' '}
              {item.template_name}
            </Text>
          </View>
          <View style={styles.rightTips}>
            <Text style={[styles.bottomText, styles.bottomLeftText]}>
        ...
        <View style={styles.scoreCharts}>
          <PercentageCircle
            ...
            <Text style={{fontSize: 14}}>
              {selectTimeName}得{studentRemarkInfo.score}分
            </Text>
          </PercentageCircle>
          ...
          <View style={styles.scoreContainer}>
            <View style={[styles.scoreView, styles.leftScore]}>
              <Text style={styles.scoreText}>
                +{studentRemarkInfo.praise_score}分
              </Text>
              <Text>表扬</Text>
            </View>
            <View style={styles.scoreView}>
              <Text style={styles.scoreText}>
                {studentRemarkInfo.criticize_score}分
              </Text>
              <Text>批评</Text>
            </View>
          </View>
