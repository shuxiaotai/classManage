  ...
  getRenderStudentGroup = () => {
    ...
    return (
      <View>
        {groupList.map(item => {
          return (
            <View style={styles.stuGroupItem} key={item.id}>
              ...
              <TouchableOpacity
                style={styles.stuGroupContentContainer}
                onPress={() => this.showStudentOfGroup(item)}>
                <Image
        ...
        <TouchableOpacity style={styles.addGroup} onPress={this.toCreateGroup}>
          <Icon name="add" color="#00aced" size={34} />
          <Text style={styles.addGroupText}>添加小组</Text>
        </TouchableOpacity>
      </View>
    );
  };
