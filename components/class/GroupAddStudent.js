  ...
  componentDidMount() {
    ...
    checkUser(() => {
      getTokenInfo().then(value => {
        fetchData
          .postData('/studentWillAddGroupList', {
            teacherId: value.id,
        ...
        <TouchableOpacity
          ...
          <Icon
            ...
            color={
              item.group_name
                ? 'gray'
                : this.state.selectStudentList.indexOf(item.id) !== -1
                ? '#3498db'
                : 'gray'
            }
          />
          ...
          />
          <Text>{item.name}</Text>
          <Text style={styles.groupNameTips}>
            {item.group_name ? `已在${item.group_name}` : ''}
          </Text>
        </TouchableOpacity>
      ))}
    </ScrollView>
  ...
  render() {
    ...
    return (
      <View style={{flex: 1}}>
        ...
        />
        {studentName === '' ? (
          this.getRenderSelectStudentList()
        ) : (
          <Text>暂无学生</Text>
        )}
      </View>
