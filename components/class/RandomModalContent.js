    ...
    return (
      <View style={styles.randomContainer}>
        <View style={styles.randomWrapper}>
          <Image
            source={
              selectStudent.avatar_url.indexOf('img') === -1
                ? {uri: getProtocol() + selectStudent.avatar_url}
                : getLocalImgIndex('student', selectStudent.avatar_url)
            }
            style={styles.randomImg}
          />
          <Text style={styles.randomText}>
            {selectStudent ? selectStudent.name : ''}
          </Text>
        </View>
        <TouchableOpacity
