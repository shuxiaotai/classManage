    ...
    return (
      <View>
        <PublicMask
          ...
          handleModal={
            isRandomVisible
              ? this.handleRandomModal
              : isStudentVisible
              ? this.handleStudentListModal
              : this.handleGroupListModal
          }
        ...
        <PublicModal
          ...
          modalLeft={
            isRandomVisible
              ? ''
              : isStudentVisible && isGroupVisible
              ? ''
              : isStudentVisible
              ? `${showRandoming ? '' : '学生主页'}`
              : '编辑小组'
          ...
          modalTitle={
            isRandomVisible
              ? '随机抽选'
              : isStudentVisible && isGroupVisible
              ? `点评${currentGroup.name}`
              : isStudentVisible
              ? `点评${currentStudent.name}`
              : `${currentGroup.name}`
          }
          renderComponent={
            isRandomVisible ? ( //显示随机抽选modal
              ...
              <RemarkModalContent
                ...
                handleGroupListModal={this.handleGroupListModal}
                handleStudentAndGroupListModal={
                  this.handleStudentAndGroupListModal
                }
                getStudentList={this.getStudentList}
                getGroupList={this.getGroupList}
              />
        ...
        <PublicHeader
          ...
          rightComponent={
            isMaster === 1 && selectKey === 2 ? (
              <Text style={{color: '#fff'}}>考勤报表</Text>
            ) : null
          }
          rightPressFun={() => navigate('CheckChart')}
        />
        <PublicTab
          tabItem={isMaster === 1 ? tabItemIsMaster : tabItem}
          selectKey={selectKey}
