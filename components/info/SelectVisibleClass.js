  ...
  renderVisibleClassList = ({item}) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={
          postNotice === 0
            ? () => this.getClassIds(item.id)
            : () => this.getTeacherClassIdArr(item.id)
        }>
              ...
              color={
                postNotice === 0
                  ? item.id === masterClassId
                    ? '#3498db'
                    : 'gray'
                  : teacherClassIdArr.indexOf(item.id) !== -1
                  ? '#3498db'
                  : 'gray'
              }
            />
      ...
      <View style={{height: '100%', position: 'relative'}}>
        <PublicHeader
          ...
          rightComponent={
            ...
            teacherClassList.length === 0 ? null : postNotice === -1 ? (
          ...
          rightPressFun={
            postNotice === -1 ? this.handleShowVisibleClass : this.cancelSelect
          }
        />
        <PublicMask
        ...
        {masterClassList.length === 0 && teacherClassList.length === 0 ? (
          ...
          <SectionList
            ...
            renderItem={({item, index, section}) =>
              this.renderVisibleClassList({item, index, section})
            }
            renderSectionHeader={({section: {title}}) => (
              <Text style={{marginVertical: 10, marginLeft: 8, fontSize: 14}}>
                {title}
              </Text>
            )}
            sections={[
