...
class Rate extends Component {
  ...
  render() {
    ...
    return (
      <ScrollView contentContainerStyle={styles.rateListContainer}>
        {rateInfo && rateInfo.rateList.length > 0
          ? rateInfo.rateList.map((item, index) => {
              if (item.length > 0) {
                return (
                  <View
                    ...
                    <View style={styles.rateTitle}>
                      <Text>
                        {moment(rateInfo.startTime).format('MM-DD')} -{' '}
                        {moment(rateInfo.endTime).format('MM-DD')}{' '}
                        {item[0].class_grade}
                        {item[0].class_name}
                      </Text>
                    </View>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        this.toRateDetail(this.getChildItemClassId(item))
                      }>
                      <Text style={styles.detailBtn}>立即查看</Text>
                    </TouchableOpacity>
                  </View>
                );
              }
