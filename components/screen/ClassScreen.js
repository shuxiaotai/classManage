...
class ClassScreen extends Component {
  ...
  static getDerivedStateFromProps(preProps, preState) {
    if (
      preProps.navigation.state.params &&
      preState.isTeacher !== preProps.navigation.state.params.isTeacher
    ) {
      return {
    ...
    if (
      preProps.navigation.state.params &&
      preState.selectIdentity !==
        preProps.navigation.state.params.selectIdentity
    ) {
      return {
    ...
    checkUser(() => {
      getTokenInfo().then(value => {
        fetchData
          ...
          .then(val => {
            setChildInfo(
              val.personInfo.childInfo ? val.personInfo.childInfo : '',
            );
            setLatestRemark(
              (val.personInfo.remark && val.personInfo.remark.length) > 0
                ? val.personInfo.remark
                : [],
            );
            setLatestCheck(
              val.personInfo.check && val.personInfo.check.length > 0
                ? val.personInfo.check
                : [],
            );
            this.setState({
          ...
          rightPressFun={
            isDeleteClass
              ? selectClassId === -1
                ...
                : this.toPostDeleteClass
              : this.handleShowManageClass
          }
        />
          ...
          <View>
            {childInfo ? (
              <View style={styles.childContainer}>
                ...
                <View style={styles.childRemarkContainer}>
                  ...
                  <View style={styles.childRemark}>
                    ...
                    {latestRemark && latestRemark.length === 0 ? (
                      <Text>本周暂无点评记录</Text>
                    ) : latestRemark.length > 0 ? (
                      latestRemark.map(item => {
                        return (
                ...
                <View style={styles.childRemarkContainer}>
                  ...
                  <View style={styles.childRemark}>
                    ...
                    {latestCheck && latestCheck.length === 0 ? (
                      <Text>本周暂无考勤记录</Text>
                    ) : (
                      <PublicImageItem
              ...
              <View>
                <PublicNoContent
                  tips={`${
                    shouldJoinClass ? '暂未加入班级' : '等待班主任通过'
                  }`}
                />
                {shouldJoinClass ? (
                  <PublicBtn tips="加入班级" onPress={this.toJoinClass} />
                ) : null}
              </View>
            )}
          </View>
