...
class addressBookScreen extends Component {
  ...
  getSections = addressBookList => {
    addressBookList.sort((a, b) => {
      return (
        pinyinUtil
          .getFirstLetter(a.username)[0]
          .toLowerCase()
          .charCodeAt(0) -
        pinyinUtil
          .getFirstLetter(b.username)[0]
          .toLowerCase()
          .charCodeAt(0)
      );
    });
    ...
    addressBookList.forEach(item => {
      addressData.forEach(addressItem => {
        if (
          addressItem.title ===
          pinyinUtil.getFirstLetter(item.username)[0].toLowerCase()
        ) {
          ...
          <View>
            <SectionList
              ...
              renderItem={({item, index, section}) =>
                this.renderAddressBook({item, index, section})
              }
